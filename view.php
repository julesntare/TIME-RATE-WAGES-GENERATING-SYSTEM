...
?>
<?php
 $sql=mysqli_query($conn,"SELECT *FROM empreg order by firstname ASC") or die(mysqli_error());
$epr='';
if(isset($_GET['epr']))
$epr=$_GET['epr'];
  if($epr=='delete')
{
  $id=$_GET['id'];
  $delete=mysqli_query("DELETE from empreg where id='$id'");
  if($delete){
...
<?php
$i=1;
$resul=mysqli_query($conn,'SELECT*from empreg order by time DESC') or die(mysqli_error());
